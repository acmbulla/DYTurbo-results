...
def parse_filename(filename):
    ...
    if order_idx is None:
        raise RuntimeError(f"Could not determine order in {filename}")

    # ── trova dove iniziano i bin ──────────────────────────────────────
    # Un token è "inizio bin" se:
    #   Case A: tokens[i] è una var pulita E tokens[i+1] ha forma var+number
    #   Case B: tokens[i] ha forma var+number
    # Tutto prima è parte del processo.

    def is_bin_start(tokens, i, order_idx):
        """Ritorna True se tokens[i] sembra l'inizio di una coppia di bin."""
        if i >= order_idx:
            return False

        # Case B: token corrente è tipo "qt0", "ptl25"
        m = re.match(r"([a-zA-Z]+)([-+]?[0-9]*\.?[0-9]+)$", tokens[i])
        if m and i + 1 < order_idx:
            return True

        # Case A: token corrente è var pulita, token+1 è var+number con stessa radice
        if i + 2 < order_idx:
            var_candidate = normalize_var(tokens[i])
            m2 = re.match(r"([a-zA-Z]+)([-+]?[0-9]*\.?[0-9]+)$", tokens[i+1])
            if m2 and normalize_var(m2.group(1)) == var_candidate:
                return True

        return False

    # Trova il primo token che sembra un bin
    bin_start_idx = 1
    for j in range(1, order_idx):
        if is_bin_start(tokens, j, order_idx):
            bin_start_idx = j
            break

    # Il processo è tutto fino a bin_start_idx
    process = "_".join(tokens[:bin_start_idx])
    ...
    while i < order_idx:
        ...
        if i + 2 < order_idx:
            ...
            m = re.match(r"([a-zA-Z]+)([-+]?[0-9]*\.?[0-9]+)$", low_token)
            if m and normalize_var(m.group(1)) == var_candidate:
                bins[var_candidate] = (float(m.group(2)), float(high_token))
        ...
        if i + 1 < order_idx:
            ...
            if m:
                ...
                bins[var] = (float(m.group(2)), float(high_token))
    ...
    for tok in tokens[order_idx + 1:]:
        m = re.match(r"([a-zA-Z]+)([-+]?[0-9]*\.?[0-9]+)", tok)
        if m:
