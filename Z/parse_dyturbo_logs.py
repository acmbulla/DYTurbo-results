...
def extract_single_bin_results(text, n_contrib, warn=True, path=""):
    ...
    with the same numbers again. Returns (pt_lo, pt_hi, results) -- results
    being the first n_contrib (value, error, exp, time, unit) matches --
    or None if this format isn't detected (no pT(l) boundary, or not
    enough matches)."""
    ...
    all_matches = RESULT_RE.findall(table_text)
    if len(all_matches) < n_contrib:
        return None

    if warn and len(all_matches) > n_contrib:
        print(f"[info] {path}: single-bin table format detected "
              f"(pT(l) = [{pt_lo}, {pt_hi}] from Constant boundaries); "
              f"using the first {n_contrib} of {len(all_matches)} "
              f"matches found (the rest is the duplicated summary line at "
              f"the bottom of the table)")

    return pt_lo, pt_hi, all_matches[:n_contrib]
...
def parse_row(row_text):
    """Extract the 5 finalized (value, error, exp, time, unit) results from
    one pT(l) row, in column order (Resummation, Counter term, V+J Real,
    V+J Virtual, TOTAL). Vegas iteration dumps in between are ignored
    automatically since they don't match RESULT_RE (they use '+-', not
    '±', and have no trailing '(...s/mi/hr)')."""
    matches = RESULT_RE.findall(row_text)
    return matches  # list of (value_str, error_str, exp_str, time_str, unit)
...
def parse_log_file(path, warn=True):
    ...
    # entries: list of (pt_lo, pt_hi, results, diag_text) -- diag_text is
    # what gets scanned for nan/inf if results doesn't have len(contributions) entries.
    entries = []
    ...
    if row_chunks:
        ...
        for pt_lo, pt_hi, row_text in row_chunks:
            entries.append((pt_lo, pt_hi, parse_row(row_text), row_text))
    else:
        ...
        if len(results) != len(contributions):
            if warn:
                ...
                for hint in diagnose_row_mismatch(diag_text, len(results), len(contributions)):
                    print(f"         -> {hint}")
            continue
        for contrib, (val, err, exp_s, tmin, tunit) in zip(contributions, results):
            ...
            if exp is not None:
                ...
                err *= 10.0 ** exp
            if tunit == "s":
                tmin = tmin / 60.0  # normalize seconds to minutes
            elif tunit == "hr":
                tmin = tmin * 60.0  # normalize hours to minutes
